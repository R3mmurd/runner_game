...
class Stage:
    ...
    def update(self, dt: float) -> None:
        if self.block_timer >= settings.TIME_BETWEEN_BLOCKS:
            x = 0
            y = settings.WINDOW_HEIGHT - settings.GROUND_HEIGHT - settings.BLOCK_HEIGHT * random.uniform(1, 3)
            self.blocks.append(settings.Block(x, y, settings.BLOCK_WIDTH, settings.BLOCK_HEIGHT))
