...
class Character:
    def __init__(self, stage: Stage) -> None:
        ...
        self.y = (
            settings.WINDOW_HEIGHT - settings.GROUND_HEIGHT - settings.CHARACTER_HEIGHT
        )
    ...
    def get_collision_box(self) -> CollisionBox:
        return CollisionBox(
            self.x + settings.CHARACTER_COLLISION_BOX_X,
            self.y + settings.CHARACTER_COLLISION_BOX_Y,
            settings.CHARACTER_COLLISION_BOX_WIDTH,
            self.height - settings.CHARACTER_COLLISION_BOX_Y,
        )
    ...
    def update(self, dt: float) -> None:
        ...
        if not self.on_ground:
            ...
            if self.vy > 0 and self.get_collision_box().collides_with(
                self.stage.get_collision_box()
            ):
                self.on_ground = True
                self.y = (
